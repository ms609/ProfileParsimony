...
#  system.time(PrepareDataProfile(congreveLamsdellMatrices[[1]], 43210, warn=FALSE))
...
test_that("Profile score calculated correctly", {
  ...
  tree <- referenceTree
  set.seed(0)
  dataset <- PrepareDataProfile(congreveLamsdellMatrices[[1]], 50000, warn=FALSE)
  score <- ProfileScore(tree, dataset)
  cat(score)
  expect_true(-355 < score && score < -347) # Range as concavity constants generated by approximation
})
